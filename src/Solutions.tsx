    ...
    <PageWrapper>
      <div className="flex flex-col lg:flex-row min-h-[90vh]">
        ...
        <div ref={contentRef} className="flex-1 px-6 lg:px-12 py-12 space-y-32">
          {solutions.map((s, idx) => {
            // UPVC has its own dedicated page — show a redirect card instead
            if (s.id === 'upvc') {
              return (
                <section key={s.id} id={s.id} className="solution-section flex flex-col gap-12 group md:flex-row">
                  <div className="w-full md:w-1/2 overflow-hidden shadow-2xl relative">
                    <img className="w-full h-[500px] object-cover group-hover:scale-105 transition-transform duration-500"
                      src={s.img} alt={s.title} loading="lazy" />
                    <div className="absolute inset-0 bg-slate-950/40" />
                  </div>
                  <div className="w-full md:w-1/2 flex flex-col justify-center">
                    <span className="text-xs font-bold text-electric-blue uppercase tracking-widest block mb-3">Dedicated Division</span>
                    <h2 className="text-4xl font-bold text-primary mb-6 uppercase tracking-tight leading-none">{s.title}</h2>
                    <p className="text-lg text-slate-500 mb-8 border-l-2 border-electric-blue pl-6">
                      Our UPVC division has a dedicated page with full product details, technical specifications,
                      and our aluplast prima-slide & IDEAL 1000 systems.
                    </p>
                    <button
                      onClick={() => navigate('/upvc')}
                      className="w-fit flex items-center gap-3 bg-electric-blue text-white px-10 py-4 text-xs font-bold uppercase tracking-widest hover:scale-105 transition-transform duration-200"
                    >
                      View UPVC Division →
                    </button>
                  </div>
